  ...
  useEffect(() => {
    async function getHomes() {
      ...
      try {
        ...
        let notRented = data.homes.filter(
          (home) =>
            home.rented !== true ||
            home.rentBy === localStorage.getItem("userId")
        );
  ...
  return (
    <Layout>
      ...
      {error && (
        <div
          className="error"
          style={{ color: "#999", marginTop: "5rem", marginBottom: "1rem" }}
        >
          {error}
        </div>
      )}
      <div className="Container_addhome">
        <section>
          ...
          <div className="advert">
            <aside
              className="video"
              style={{ display: play ? "hidden" : "block" }}
            >
              <video
                src="demo.mp4"
                id="video"
            ...
            <aside className="news">
              ...
              <div>
                From : {fromApi.query === undefined ? " " : fromApi.query.from}
              </div>
              <div>
                To : {fromApi.query === undefined ? " " : fromApi.query.to}
              </div>
              <div>
                Date: {fromApi.query === undefined ? " " : fromApi.date}
              </div>
              <div>
                Rate : {fromApi.query === undefined ? " " : fromApi.result}
              </div>
            </aside>
          </div>
        </section>
